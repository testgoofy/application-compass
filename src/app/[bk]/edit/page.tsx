...
export default async function EditPosition({
  params,
}: {
  params: Promise<{ bk: string }>;
}) {
  const { bk } = await params;

  const position = await Database.getInstance().position.findFirst({
    where: {
      bk: bk,
      valid_to: null,
    },
  ...
  const statuses = await Database.getInstance().processNode.findMany();

  async function handler(formData: FormData) {
    'use server';

    await Database.getInstance().position.update({
      where: {
        id: position?.id,
      },
      data: {
    ...
    await Database.getInstance().position.create({
      data: {
        bk: bk,
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between">
          ...
          <div className="flex gap-2">
            <button
              type="submit"
              className="flex justify-between items-center gap-1 bg-green-500 text-white p-1 pr-2 rounded-md"
            >
        ...
        <div className="flex flex-col border border-gray-300 divide-y divide-gray-300 rounded-lg px-2">
          ...
          <div className="flex content-end py-1">
            <div className="flex items-center w-1/2">
              ...
              <p className="text-red-500 font-semibold pl-1">*</p>
            </div>
            <TextInput
              id="title"
              title=""
          ...
          <div className="flex content-end py-1">
            ...
            <TextInput
              id="degree"
              title=""
              value={position?.degree == null ? undefined : position.degree}
              className="w-1/2"
              placeholder="80 - 100%"
            />
          </div>
          <div className="flex content-end py-1">
            ...
            <TextInput
              id="begin"
              title=""
              value={position?.begin == null ? undefined : position.begin}
              className="w-1/2"
          ...
          <div className="flex content-end py-1">
            ...
            <TextInput
              id="duration"
              title=""
              value={position?.duration == null ? undefined : position.duration}
              className="w-1/2"
          ...
          <div className="flex content-end py-1">
            ...
            <NumberInput
              id="salary"
              title=""
              value={position?.salary == null ? undefined : position.salary}
              className="w-1/2"
              placeholder="100000"
              suffix="CHF"
            />
        ...
        <div className="flex flex-col border border-gray-300 rounded-lg px-2">
          ...
          <div className="flex content-end py-1">
            ...
            <select
              name="status"
              id="status"
              defaultValue={position?.stateId}
              className="w-1/2 rounded border-1 bg-white"
            >
        ...
        <PositionTab
          ...
          description={
            position?.description != null ? position.description : undefined
          }
          requirements={
            position?.requirements != null ? position.requirements : undefined
          }
        />
      </div>
    </Form>
